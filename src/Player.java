...
public class Player {
   ...
   public PokerHand evaluate(ArrayList<Card> comunityCards) {                                                //checks what the best pokerhand is that the player can have with his current cards
      ...
      for (int i = 0; i < 7; i++) {                                                                                  //removes every combination of 2 cards from the 7 possible cards to get every combination of 5 cards (21)
         for (int j = i + 1; j < 7; j++) {
            ...
            if (currentHandType.getHandValue() > bestHand.getHandValue() || (currentHandType.getHandValue() == bestHand.getHandValue() && winsTiebreak(lastTiebreakValue, currentTiebreakeValues))) {  // if the current hand is better than the previous best it becomes the new best
   ...
   private int[] getTiebreakValuesForHand(PokerHand hand, ArrayList<Card> workingHand) {                    //gives out an int[] with all the needed Tiebrake values. If a Hand needs less that 5 values, the rest is set to 0
      int[] tiebreaker = new int[5];
      if (hand == PokerHand.STRAIGHT_FLUSH || hand == PokerHand.STRAIGHT) {   
         if (workingHand.get(0).getValue() == 14 && workingHand.get(1).getValue() == 5) {        //special case where the ace is a 1
            tiebreaker[0] = 5;
         } else {
            tiebreaker[0] = workingHand.get(0).getValue();                                             // sets the first index of the array to the highest card in the straight
         }
      }
      ...
      else if (hand == PokerHand.FLUSH || hand == PokerHand.HIGHCARD) {                                     //the tiebreaker values in case of flush and highcard are the same : all cards are needed
         for (int i = 0; i < 5; i++)
         ...
         int remainingCard = -1;
         for (int i = 0; i < 4; i++) {
            if (workingHand.get(i).getValue() == workingHand.get(i + 1).getValue()) {                 //highpair is set to the first pair that is encountered, lowpair is set to the second
               if (highPair == -1)
   ...
   private boolean isStraight(ArrayList<Card> workingHand) {
      if (workingHand.get(0).getValue() == 14 && workingHand.get(1).getValue() == 5 && workingHand.get(2).getValue() == 4 && workingHand.get(3).getValue() == 3 && workingHand.get(4).getValue() == 2) { //special case where the ace is a 1 
         return true; 
      }
